...
function Resorts() {
        ...
        const fetcher = async () => {
            ...
            if (user){
                ...
                if (!page){
                    ...
                    let requestNextPage = await fetch(`http://localhost:8888/allMemResortProducts/${2}`, {
                        headers: {Authorization: 'Bearer ' + token},
                    })
                    ...
                    let request = await fetch(`http://localhost:8888/allMemResortProducts/${pageNumber}`, {
                        headers: {Authorization: 'Bearer ' + token},
                    })
                    let result = await request.json();
                    setData(result);
    ...
    const goNextPage = () => {
        if (page){
            window.location.replace(`/resorts/${pageNumber+1}`)
        }
        else {
            window.location.replace(`/resorts/2`)
        }
    }
